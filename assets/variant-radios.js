...
class VariantRadios extends HTMLElement {
  ...
  renderProductInfo() {
    ...
    fetch(`${this.dataset.url}?variant=${requestedVariantId}&section_id=${this.dataset.originalSection}`)
      ...
      .then((responseText) => {
        ...
        // Mettre à jour le bouton Buy Now selon la dispo
  ...
  getVariantData() {
    ...
    const jsonEl = document.getElementById(`product-json-${this.dataset.section}`);
    try {
    ...
    if (window.productVariants && Array.isArray(window.productVariants)) {
      this.variantData = window.productVariants;
      return this.variantData;
    }
    return [];
  }
